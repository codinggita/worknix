...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const userExists = await (type === "employee" ? User : Company).findOne({ email });
    if (userExists) {
      return res.status(400).json({ message: "User already exists" });
    }
    ...
    if (type === "employee") {
      ...
      newUser = new User({ username, email, password: hashedPassword, userId });
    } else {
      const companyId = await generateUniqueId("WC", Company, "companyId");
      newUser = new Company({ companyName: username, email, password: hashedPassword, companyId });
    }

    await newUser.save();
    const token = jwt.sign({ id: newUser._id, type }, JWT_SECRET, { expiresIn: "1h" });
...
router.post("/login", async (req, res) => {
  ...
  try {
    const user = await (type === "employee" ? User : Company).findOne({ email });
