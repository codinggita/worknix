...
const Lefts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send form data to backend
      const res = await axios.post("https://worknix-lgin-and-signup.onrender.com/signup", formData);

      if (res.data.message === "User already exists") {
        alert("User already exists");
      } else if (res.data.message === "User registered successfully") {
        // Automatically login after successful registration
        const loginRes = await axios.post("https://worknix-lgin-and-signup.onrender.com/login", {
          email: formData.email,
          password: formData.password,
        });
